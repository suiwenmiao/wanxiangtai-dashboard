...
下载弹窗必须选择「昨天」；如果页面仍显示「过去 7 天」，脚本会停止，避免下载多日报表。

商品报表必须从「商品数据明细」下载，并在下载前把「维度」设置为「全选」。下载弹窗时间粒度必须是「分天」。

商品报表必须包含 `日期` 和 `主体ID`。场景/计划列可能因平台导出维度不同而缺失，脚本会补为「未分类 / 未关联计划」后继续处理。
